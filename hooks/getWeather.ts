...
function getWeather({
  ...
  setLocalTime,
  setRain,
}: GetWeatherProps): void {
  if (!inputCity && !lat && !lon) return;
  const API_KEY: string = "d551f5d509a0045cf8cdb61648300237";

  if (inputCity && !lat && !lon)
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${inputCity.trim()}&units=metric&lang=ru&appid=${API_KEY}`
    )
      ...
      .then((json: WeatherDataObject) => {
        ...
        if (json.rain && json.rain["1h"] > 0.6)
          rain = Math.round(json.rain["1h"]).toString();
