...
function getWeather({ inputCity, lon, lat, setCity, setDegreesCelsias, setWeather, setAutoLocation, setHumidity, setLocalTime }: GetWeatherProps): void {
    ...
    if (inputCity && !lat && !lon)
        //fetch(`https://api.openweathermap.org/data/2.5/weather?q=${inputCity}&units=metric&lang=ru&appid=${API_KEY}`, { headers: {} })
        fetch(`https://api.weather.yandex.ru/v2/informers?lat=55.75396&lon=37.620393&lang=ru_RU`, { headers: {
            "X-Yandex-API-Key": "bfe913d5-c1c0-4490-8722-b8e27c0715f8"
        }})
